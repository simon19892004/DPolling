...
TransparentNanoMacEntity::Send (Ptr<Packet> p, uint32_t dst)
{
  ...
  //NS_LOG_FUNCTION("NodeID "<<GetDevice ()->GetNode ()->GetId ()<<"Send pkt: "<<p->GetUid()<<"neighbour-size: "<<m_neighbors.size());
  NanoMacHeader header;
  uint32_t src = GetDevice ()->GetNode ()->GetId ();
  header.SetSource (src);
  header.SetDestination (dst);
...
{
  //NS_LOG_FUNCTION ("MAC receive! NodeID "<<GetDevice()->GetNode()->GetId()<<"pktID: "<<p->GetUid());
}
